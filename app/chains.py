...
class GeneralSignerChains(enum.Enum):
    ...
    Polygon = Chain(
        name="polygon",
        native_asset="MATIC",
        crypto_asset="polygon",
        decimal=18,
        chain_id=137,
        gql_graph_name="Polygon",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.POLYGON_MAINNET_RPC_ENDPOINT,
    )
    Avalanche = Chain(
        name="avalanche",
        native_asset="AVAX",
        crypto_asset="avalanche",
        decimal=18,
        chain_id=43114,
        gql_graph_name="Avalanche",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.AVALANCHE_MAINNET_RPC_ENDPOINT,
    )
    ...
    BinanceSmartChain = Chain(
        ...
        decimal=18,
        chain_id=56,
        gql_graph_name="BinanceSmartChain",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.BSC_MAINNET_RPC_ENDPOINT,
    )
    ...
    Fantom = Chain(
        ...
        decimal=18,
        chain_id=250,
        gql_graph_name="Fantom",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.FANTOM_MAINNET_RPC_ENDPOINT,
    )

    Arbitrum = Chain(
        name="arbitrum",
        native_asset="ETH",
        crypto_asset="arbitrum",
        decimal=18,
        chain_id=42161,
        gql_graph_name="Arbitrum",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.ARBITRUM_MAINNET_RPC_ENDPOINT,
    )

    Optimism = Chain(
        name="optimism",
        native_asset="ETH",
        crypto_asset="optimism",
        decimal=18,
        chain_id=10,
        gql_graph_name="Optimism",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.OPTIMISM_RPC_ENDPOINT,
    )
    ...
    Gnosis = Chain(
        ...
        decimal=18,
        chain_id=100,
        gql_graph_name="gnosis",
        chain_type=GeneralSignerChainType.EVMBased,
        rpc_endpoint=settings.GNOSIS_MAINNET_RPC_ENDPOINT,
    )
    ...
    def get_chains_by_type(
        cls, type: GeneralSignerChainType
    ) -> List["GeneralSignerChains"]:
    ...
    def get_chain_by_crypto_asset(cls, crypto_asset: str) -> "GeneralSignerChains":
        return next(
            (chain for chain in cls if chain.value.crypto_asset == crypto_asset), None
        )
